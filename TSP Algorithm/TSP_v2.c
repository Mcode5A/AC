...
float compute_path_distance(int* path)
{
    ...
    for (i = 1; i < NUM_CITIES; i++)
        dist = dist + distances[path[i-1]][path[i]]; //distance(Cities[path[i - 1]], Cities[path[i]]);
    return dist + distances[path[NUM_CITIES-1]][path[0]]; //distance(Cities[path[NUM_CITIES - 1]], Cities[path[0]]);
}
...
void merge(chrom* __restrict A, int szA, chrom* __restrict B, int szB, chrom* __restrict C)
{
    int i = 0, j = 0;
    while (i + j < szA + szB)
    {
...
void copy_population(chrom* __restrict P_in, chrom* __restrict P_out, int N)
{
    int i, j;
    for (i = 0; i < N; i++)
        for (j = 0; j < NUM_CITIES; j++)
...
void mate(chrom* __restrict P_in, int Nin, chrom* __restrict P_out, int Nout)
{
...
int main(int argc, char** argv)
{
    ...
    for (i = 0; i < EPOCHS; i++)
    {
        ...
        copy_population(TempPopulation+ELITSIZE, Population+ELITSIZE, POPSIZE-ELITSIZE);
        compute_fitness(Population, POPSIZE);
