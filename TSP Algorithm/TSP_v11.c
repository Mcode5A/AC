...
int main(int argc, char** argv)
{
    ...
    #pragma acc data copyin(distances[0:NUM_CITIES][0:NUM_CITIES], Population[0:POPSIZE])
    {
    for (i = 0; i < EPOCHS; i++)
    {
        ...
        // COMPUTE_PATH_DISTANCE
         ...
         for (int k = 0; k < POPSIZE; k++){
           ...
           for (int j = 1; j < NUM_CITIES; j++) 
              dist = dist + distances[gen[Population[k].index][j-1]][gen[Population[k].index][j]];
           //Population[k].distance = dist+distances[gen[Population[k].index][249]][gen[Population[k].index][0]];
