...
int main (int argc, char **argv)
{
  ...
  for (i=0; i<EPOCHS; i+=2)
  {
   ...
   // copy_population ( TempPopulation, Population, POPSIZE );
    compute_fitness ( TempPopulation+ELITSIZE, POPSIZE-ELITSIZE );
    merge_sort      ( TempPopulation, POPSIZE );  // sort population by lower fitness, to generate new elite

    
    copy_population ( TempPopulation, Population, ELITSIZE );    // copy elite population to new generation
    mate            ( TempPopulation, ELITSIZE, Population+ELITSIZE, POPSIZE-ELITSIZE ); // mate from elite
    mutate          ( Population+ELITSIZE, POPSIZE-ELITSIZE );  // do not affect elite
  //  copy_population ( TempPopulation, Population, POPSIZE );
    compute_fitness ( Population+ELITSIZE, POPSIZE-ELITSIZE );
