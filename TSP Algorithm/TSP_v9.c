...
unsigned char gen2[POPSIZE][NUM_CITIES];
...
void mate(chrom* P_in, int Nin, chrom* P_out, int Nout)
{
    ...
    #pragma omp parallel for  num_threads(8) proc_bind(close) shared(P_out, P_in)
    for (int m = 0; m < Nout; m++)
    {
        ...
        for (int i = 1; i < NUM_CITIES; i++)
            dist = dist + distances[P_out[m].gen[i - 1]][P_out[m].gen[i]];
...
void init_random_array(int N) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        Random[2][i] = myRandom() % NUM_CITIES;
    }

    for (int i = 0; i < N; i++) {
        Random[3][i] = myRandom() % NUM_CITIES;
        Random[4][i] = myRandom() % NUM_CITIES;
